...
function loadFrame(tab){const src=getFileSrc(tab);const frameId='page-'+tab;const frame=document.getElementById(frameId);const ph=document.getElementById('ph-'+tab);const phSub=document.getElementById('ph-'+tab+'-sub');if(!frame)return;if(ph){const _m=ph.querySelector('.msg');if(_m)_m.textContent='파일 로딩중...';}
...
function switchThemeMapTab(sub,btn){document.querySelectorAll('#kr-thememap-bar .stab').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');const wMap=document.getElementById('wrap-kr-thememap');const wTheme=document.getElementById('wrap-kr-theme');document.getElementById('wrap-newhigh').style.display='none';if(sub==='map'){if(wTheme)wTheme.style.display='none';if(wMap)wMap.style.display='block';loadFrame('kr-thememap');}else{_krThemeTab=sub;if(wMap)wMap.style.display='none';if(wTheme)wTheme.style.display='block';if(toYMD(currentDate)>=TODAY_YMD&&!_krThemeExactSrc()){_showKrThemeEmpty(sub);return;}
...
function _krThemeExactSrc(){const list=BINDEX[TAB_KEY['kr-theme']]||[];const ymd=toYMD(currentDate);for(const e of list){if(entryLabelToYMD(e.label)===ymd)return e.src;}
return null;}
