...
function switchKrThemeTab(tab,btn){_krThemeTab=tab;document.querySelectorAll('#kr-theme-bar .stab').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');const wrapTheme=document.getElementById('wrap-kr-theme');const wrapHigh=document.getElementById('wrap-newhigh');if(tab==='stk-high'||tab==='etf-high'||tab==='rank-chg'||tab==='rank-val'){wrapTheme.style.display='none';wrapHigh.style.display='block';if(tab==='rank-chg')renderRankChange();else if(tab==='rank-val')renderRankValue();else renderNewHigh(tab);}else{wrapTheme.style.display='block';wrapHigh.style.display='none';_applyKrThemeSection(tab);}}
...
async function renderRankChange(){const el=document.getElementById('newhigh-content');el.innerHTML='<div style="color:var(--muted);font-size:19.5px;">로딩 중...</div>';try{const[ks,kq]=await Promise.all([fetch('/api/rank_change_kospi.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),fetch('/api/rank_change_kosdaq.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),]);const row=(r,i)=>{const live=_getPrice(r.itm_cd);const prc=(live&&live.curr_prc)||r.curr_prc;const u=live&&live.ud_rt!=null?live.ud_rt:r.ud_rt;const ud=_rkUd(u);const nm_esc=(r.itm_nm||'').replace(/'/g,"&#39;");return`<tr style="cursor:pointer;border-bottom:1px solid var(--border);"
          onmouseover="this.style.background='var(--card2)'" onmouseout="this.style.background=''"
          onclick="openKrStock('${r.itm_cd}','${nm_esc}')">
...
async function renderRankValue(){const el=document.getElementById('newhigh-content');el.innerHTML='<div style="color:var(--muted);font-size:19.5px;">로딩 중...</div>';try{const[valD,turnD]=await Promise.all([fetch('/api/rank_value.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),fetch('/api/rank_turnover.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),]);const isTurn=_rankValMode==='turnover';const data=isTurn?turnD:valD;const rows=data.rows||[];const mkBadge=(mkt)=>{const kospi=mkt==='KOSPI';return`<span style="font-size:11px;padding:1px 5px;border-radius:3px;font-weight:700;
