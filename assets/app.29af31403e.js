...
async function loadNewHighSection(){const el=document.getElementById('home-newhigh');try{const res=await fetch('/moneyweb/api/new_high_stocks.json?t='+Date.now());const data=await res.json();const rows=data.rows||[];if(rows.length===0){el.innerHTML='<div style="color:var(--muted);font-size: 18px;">데이터 없음</div>';return;}
