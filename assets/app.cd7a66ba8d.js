var ML_ADMIN_EMAILS=['saymeany@gmail.com','good.moneyland@gmail.com'];function _mlIsAdmin(){try{var u=JSON.parse(localStorage.getItem('ml_user')||'null');return!!(u&&u.email&&ML_ADMIN_EMAILS.indexOf(String(u.email).trim().toLowerCase())>=0);}catch(e){return false;}}
...
if(e.key==='Escape'){const gm=document.getElementById('glmodal');if(gm&&gm.classList.contains('open'))glClose();}});function switchTab(tab,el){document.querySelectorAll('.ntab, .ntab-home').forEach(t=>t.classList.remove('active'));if(el&&el.classList)el.classList.add('active');currentTab=tab;document.getElementById('subtab-bar').style.display=(tab==='schedule')?'flex':'none';document.getElementById('kr-theme-bar').style.display=(tab==='kr-theme')?'flex':'none';document.getElementById('kr-thememap-bar').style.display=(tab==='kr-thememap')?'flex':'none';if(tab!=='kr-theme')document.getElementById('wrap-newhigh').style.display='none';document.getElementById('date-nav').style.visibility='visible';document.querySelectorAll('.page-inner').forEach(p=>p.classList.remove('active'));['us-morning','kr-morning','report','per','peer','kr-theme','kr-marketmap','global-marketmap','global-thememap','kr-thememap','kr-sugup','dram'].forEach(k=>{const w=document.getElementById('wrap-'+k);if(w)w.style.display='none';});if(tab==='home'){document.getElementById('page-home').classList.add('active');loadHomeSummary();}
...
function getFileSrc(tab){const key=TAB_KEY[tab];if(!key||!BINDEX[key])return null;if(tab==='per'&&BINDEX[key].length)return BINDEX[key][0].src;const ymd=toYMD(currentDate);const list=BINDEX[key];for(const entry of list){const lblYMD=entryLabelToYMD(entry.label);if(lblYMD===ymd)return entry.src;}
...
function loadFrame(tab){const src=getFileSrc(tab);const frameId='page-'+tab;const frame=document.getElementById(frameId);const ph=document.getElementById('ph-'+tab);const phSub=document.getElementById('ph-'+tab+'-sub');if(!frame)return;if(!src){if(ph)ph.style.display='flex';frame.style.display='none';frame.src='';if(phSub)phSub.textContent=toLabel(currentDate)+' 파일 없음';return;}
...
function switchKrThemeTab(tab,btn){_krThemeTab=tab;document.querySelectorAll('#kr-theme-bar .stab').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');const wrapTheme=document.getElementById('wrap-kr-theme');const wrapHigh=document.getElementById('wrap-newhigh');if(tab==='stk-high'||tab==='etf-high'){wrapTheme.style.display='none';wrapHigh.style.display='block';renderNewHigh(tab);}else{wrapTheme.style.display='block';wrapHigh.style.display='none';_applyKrThemeSection(tab);}}
function switchThemeMapTab(sub,btn){document.querySelectorAll('#kr-thememap-bar .stab').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');const wMap=document.getElementById('wrap-kr-thememap');const wTheme=document.getElementById('wrap-kr-theme');document.getElementById('wrap-newhigh').style.display='none';if(sub==='map'){if(wTheme)wTheme.style.display='none';if(wMap)wMap.style.display='block';loadFrame('kr-thememap');}else{_krThemeTab=sub;if(wMap)wMap.style.display='none';if(wTheme)wTheme.style.display='block';const want=getFileSrc('kr-theme');const f=document.getElementById('page-kr-theme');if(f&&want&&f.getAttribute('data-srckey')!==want){f.setAttribute('data-srckey',want);loadFrame('kr-theme');}else{_applyKrThemeSection(sub);}}}
...
+'<table style="border-collapse:collapse;"><tbody>'+trs+'</tbody></table></div>';};const detailCards=detailSorted.map(([s,it])=>buildStkCard(s,it)).join('');summaryHtml='<div style="display:grid;grid-template-columns:160px 160px 1fr;gap:12px;margin-bottom:16px;align-items:start;">'
+'<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:10px;">'
...
loadThemeToday();loadNewHighSection();loadDrBinance();}
...
async function loadNewHighSection(){const el=document.getElementById('home-newhigh');try{const res=await fetch('/api/new_high_stocks.json?t='+Date.now());const data=await res.json();const rows=data.rows||[];if(rows.length===0){el.innerHTML='<div style="color:var(--muted);font-size: 18px;">데이터 없음</div>';return;}
...
if(!GL_PEER)await ensureGlPeer();const dd=document.getElementById('gl-search-dropdown');if(!dd)return;_glLastQuery=q;_renderGlSearch(q,dd);ensureGlLive().then(()=>{if(_glLastQuery===q&&dd.classList.contains('open'))_renderGlSearch(q,dd);});}
...
dd.classList.add('open');return;}
const prevSel=_glSelIdx;dd.innerHTML=results.map(({tk,d})=>{const nm=(d.n||tk).replace(/</g,'&lt;');const live=GL_LIVE[tk];const chg=(live&&live.c!=null)?live.c:(d.chg!=null?d.chg:null);const udClr=chg!=null?(chg>0?'#ef4444':chg<0?'#3b82f6':'#94a3b8'):'#94a3b8';const hasExt=!!(live&&live.e!=null);const extClr=hasExt?(live.e>=0?'#ef4444':'#3b82f6'):'';const extHtml=hasExt?`<span class="sd-ext">(시간외 <span style="color:${extClr}">${live.e >= 0 ? '+' : ''}${Number(live.e).toFixed(2)}%</span>)</span>`:'';const regHtml=chg!=null?`<span class="sd-tp" style="color:${udClr}">${chg > 0 ? '▲' : chg < 0 ? '▼' : ''}${Math.abs(chg).toFixed(2)}%</span>`:`<span class="sd-tp" style="color:#94a3b8">—</span>`;return`<div class="sd-item" data-tk="${tk.replace(/"/g,'&quot;')}" onclick="(function(el){closeGlSearchDropdown();document.getElementById('gl-search-input').value='';glPopup(el.dataset.tk);})(this)">
...
function switchGhCountry(code){GH_CUR=code;renderGhTabs();renderGlobalHigh();const p=document.getElementById('page-global-high');if(p)p.scrollTop=0;}
...
async function ensureTgtprcData(){if(TGTPRC_DATA)return TGTPRC_DATA;TGTPRC_DATA={};TGTPRC_MONTHLY={};try{const list=BINDEX['kr-tgtprc'];if(!list||!list.length)return TGTPRC_DATA;const res=await fetch(list[0].src);const html=await res.text();const lines=html.split('\n');const sLine=lines.find(l=>/^\s*(?:const|var|let)\s+STOCKS\s*=/.test(l));const mLine=lines.find(l=>/^\s*(?:const|var|let)\s+MONTHLY\s*=/.test(l));if(sLine){const stocks=new Function(sLine.trim()+'; return STOCKS;')();stocks.forEach(s=>{TGTPRC_DATA[s.itm_cd]=s;});}
...
async function openKrStock(cd,nm=''){await Promise.all([ensureTgtprcData(),ensureKrStockMeta(),ensureKrReports()]);_spRender(cd,_buildPopupData(cd,nm));document.getElementById('sp-overlay').style.display='block';document.body.style.overflow='hidden';}
