...
async function renderRankCombined(){const el=document.getElementById('newhigh-content');el.innerHTML='<div style="color:var(--muted);font-size:19.5px;">로딩 중...</div>';try{const[ks,kq,valD,turnD]=await Promise.all([fetch('/api/rank_change_kospi.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),fetch('/api/rank_change_kosdaq.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),fetch('/api/rank_value.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),fetch('/api/rank_turnover.json?t='+Date.now()).then(r=>r.json()).catch(()=>({rows:[]})),]);const chgRow=(r,i)=>{const live=_getPrice(r.itm_cd);const prc=(live&&live.curr_prc)||r.curr_prc;const u=live&&live.ud_rt!=null?live.ud_rt:r.ud_rt;const ud=_rkUd(u);const nm_esc=(r.itm_nm||'').replace(/'/g,"&#39;");return`<tr style="cursor:pointer;border-bottom:1px solid var(--border);"
          onmouseover="this.style.background='var(--card2)'" onmouseout="this.style.background=''"
          onclick="openKrStock('${r.itm_cd}','${nm_esc}')">
        ...
        </table></div>`;};const isTurn=_rankValMode==='turnover';const vd=isTurn?turnD:valD;const vrows=vd.rows||[];const mkBadge=(mkt)=>{const kospi=mkt==='KOSPI';return`<span style="font-size:11px;padding:1px 5px;border-radius:3px;font-weight:700;
        color:${kospi?'#2563eb':'#059669'};background:${kospi?'rgba(37,99,235,.12)':'rgba(5,150,105,.12)'};">${mkt||'-'}</span>`;};const valRow=(r,i)=>{const live=_getPrice(r.itm_cd);const prc=(live&&live.curr_prc)||r.curr_prc;const u=live&&live.ud_rt!=null?live.ud_rt:r.ud_rt;const ud=_rkUd(u);const nm_esc=(r.itm_nm||'').replace(/'/g,"&#39;");const metric=isTurn?(r.turnover!=null?Number(r.turnover).toFixed(2)+'%':'-'):_rkKrw(r.trd_amt);const sub=isTurn?_rkKrw(r.trd_amt):_rkKrw(r.mktcap);return`<tr style="cursor:pointer;border-bottom:1px solid var(--border);"
          onmouseover="this.style.background='var(--card2)'" onmouseout="this.style.background=''"
          onclick="openKrStock('${r.itm_cd}','${nm_esc}')">
        <td style="padding:5px 4px 5px 8px;text-align:right;font-size:13px;color:var(--muted);">${i+1}</td>
      ...
      </tr>`;};const tabBtn=(m,label)=>{const on=_rankValMode===m;return`<div onclick="setRankValMode('${m}')" style="cursor:pointer;padding:4px 12px;border-radius:7px;font-size:13.5px;font-weight:700;white-space:nowrap;
        ${on?'background:#f59e0b;color:#111;':'background:var(--card2);color:var(--muted);border:1px solid var(--border);'}">${label}</div>`;};const metricHead=isTurn?'회전율':'거래대금';const subHead=isTurn?'거래대금':'시가총액';const valCol=`<div style="border:1px solid var(--border);border-radius:10px;overflow:hidden;">
        <div style="padding:7px 12px;background:var(--card2);border-bottom:1px solid var(--border);display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          <span style="font-size:15px;font-weight:800;white-space:nowrap;">💰 거래대금 상위</span>
          ${tabBtn('value','거래대금')}${tabBtn('turnover','시총대비')}
        </div>
...
const evAll=data.events;const ev=evAll.slice(-24);const n=ev.length;const W=680,H=250,PL=10,PR=10,PT=26,PB=46;const iw=W-PL-PR,ih=H-PT-PB;const maxAmt=Math.max(...ev.map(e=>e.amt||0),1);const maxCum=Math.max(...ev.map(e=>e.cum),1);const bw=Math.max(5,Math.min(30,iw/n*0.6));const xp=bw/2+2,pw=iw-2*xp;const x=i=>PL+xp+(n===1?pw/2:pw*i/(n-1));const yB=v=>PT+ih*(1-(v||0)/maxAmt);const yC=v=>PT+ih*(1-v/maxCum);const col=e=>e.kind==='해지'?'#7f1d1d':e.kind==='정정'?'#64748b':(e.ratio!=null&&e.ratio>=10)?'#ef4444':'#3b82f6';const showLbl=n<=16;const bars=ev.map((e,i)=>{const h=Math.max(0,PT+ih-yB(e.amt));const lbl=(showLbl&&e.amt)?`<text x="${x(i).toFixed(1)}" y="${(yB(e.amt)-4).toFixed(1)}" fill="#cbd5e1" font-size="10.5" font-weight="600" text-anchor="middle">${_won억(e.amt)}</text>`:'';return`<rect x="${(x(i)-bw/2).toFixed(1)}" y="${yB(e.amt).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="${col(e)}" opacity="0.9"><title>${_esc(e.dt)} ${_esc(e.kind)} ${_won억(e.amt)}${e.content?(' · '+_esc(e.content)):''}${e.party?(' / '+_esc(e.party)):''}</title></rect>${lbl}`;}).join('');const linePts=ev.map((e,i)=>`${x(i).toFixed(1)},${yC(e.cum).toFixed(1)}`).join(' ');const step=Math.max(1,Math.ceil(n/8));const xlab=ev.map((e,i)=>(i%step===0||i===n-1)?`<text x="${x(i).toFixed(1)}" y="${(PT+ih+12).toFixed(1)}" fill="#64748b" font-size="10" text-anchor="end" transform="rotate(-40 ${x(i).toFixed(1)} ${(PT+ih+12).toFixed(1)})">${e.dt.slice(2)}</text>`:'').join('');const svg=`<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;">
     <line x1="${PL}" y1="${PT+ih}" x2="${W-PR}" y2="${PT+ih}" stroke="#1e2d47" stroke-width="1"/>
     ${bars}
     <polyline points="${linePts}" fill="none" stroke="#f59e0b" stroke-width="1.5" opacity="0.85"/>
     ${xlab}
     <text x="${W-PR}" y="${PT-10}" fill="#f59e0b" font-size="11" text-anchor="end">누적선 → ${_won억(maxCum)}</text>
     <text x="${PL}" y="${PT-10}" fill="#64748b" font-size="11">막대=개별 수주금액</text>
   </svg>`;const rev=evAll.slice().reverse().slice(0,40);const kbadge=k=>k==='신규'?'':`<span style="font-size:11px;color:${k==='해지'?'#ef4444':'#94a3b8'};border:1px solid ${k==='해지'?'#7f1d1d':'#334155'};border-radius:4px;padding:0 4px;margin-left:4px;">${k}</span>`;const trs=rev.map(e=>`<tr style="border-bottom:1px solid #1a2740;">
    ...
    </tr>`).join('');box.innerHTML=`<div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:6px;">
       <div style="font-size:18px;color:#e2e8f0;font-weight:700;letter-spacing:.04em;">수주(공급계약) 추이</div>
       <div style="font-size:15px;color:#94a3b8;">신규 ${data.cnt}건 · 누적 <b style="color:#f59e0b;">${_won억(data.total)}</b></div>
     </div>
     <div style="background:#162033;border:1px solid #1e2d47;border-radius:8px;padding:10px 8px 2px;">${svg}
       <div style="display:flex;gap:14px;padding:4px 6px 8px;font-size:12px;color:#64748b;flex-wrap:wrap;">
         <span><span style="color:#ef4444;">▮</span> 대형수주(매출대비 10%↑)</span>
