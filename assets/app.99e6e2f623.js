...
async function _renderOrderTab(cd){const btn=document.getElementById('sp-tab-btn-order');const kpi=document.getElementById('sp-order-kpi');if(btn)btn.style.display='none';if(kpi)kpi.innerHTML='';await _ensureOrderBacklog();const data=_ORDER_BACKLOG&&_ORDER_BACKLOG[cd];if(!data||!data.events||!data.events.length)return;if(btn)btn.style.display='';_renderOrderBacklog(cd,_ORDER_BACKLOG);if(kpi){const latest=data.events[data.events.length-1];kpi.innerHTML=`<div onclick="_spGoOrder()" title="수주 상세 보기"
        ...
        <div style="white-space:nowrap;"><span style="font-size:13px;color:#94a3b8;">수주 신규누적</span>
          <b style="font-size:20px;color:#f59e0b;margin-left:8px;">${_won억(data.total)}</b>
          <span style="font-size:14px;color:#64748b;margin-left:6px;">· ${data.cnt}건</span></div>
        <div style="font-size:13px;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">최근 ${_esc(latest.content || latest.party || '')} <span style="color:#38bdf8;">›</span></div>
      </div>`;}}
