var ML_WM={id:'mlwm',afterDraw:function(c){var x=c.ctx,a=c.chartArea;if(!a)return;x.save();x.globalAlpha=0.11;x.fillStyle='#ffffff';x.font='700 '+Math.max(14,Math.min(40,(a.right-a.left)/12))+'px sans-serif';x.textAlign='center';x.textBaseline='middle';x.fillText('moneyland.co.kr',(a.left+a.right)/2,(a.top+a.bottom)/2);x.restore();}};var ML_ADMIN_EMAILS=['saymeany@gmail.com','good.moneyland@gmail.com'];function _mlIsAdmin(){try{var u=JSON.parse(localStorage.getItem('ml_user')||'null');return!!(u&&u.email&&ML_ADMIN_EMAILS.indexOf(String(u.email).trim().toLowerCase())>=0);}catch(e){return false;}}
