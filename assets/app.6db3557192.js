...
function _obBacklogSvg(bl,F){const n=bl.length;if(!n)return`<svg viewBox="0 0 ${F.W} 60"><text x="20" y="34" fill="#64748b" font-size="13">추정 불가(계약기간 정보 부족)</text></svg>`;const maxV=Math.max(...bl.map(p=>p.v),1);const y=v=>F.PT+F.ih*(1-v/maxV);const pts=bl.map(p=>`${F.xd(p.t).toFixed(1)},${y(p.v).toFixed(1)}`).join(' ');const area=`${F.xd(bl[0].t).toFixed(1)},${F.base} ${pts} ${F.xd(bl[n-1].t).toFixed(1)},${F.base}`;return`<svg viewBox="0 0 ${F.W} ${F.H}" style="width:100%;height:auto;display:block;">
     ...
     ${_obTicks(F)}</svg>`;}
function _obConfirmedSvg(ser,F){const n=ser.length;if(!n)return'';const maxV=Math.max(...ser.map(o=>o.backlog),1);const yB=v=>F.PT+F.ih*(1-v/maxV);const bw=Math.max(6,Math.min(46,F.monthPx*3*0.7));const bars=ser.map(o=>{const x=F.xd(o.ms!=null?o.ms:_periodMs(o.period));const h=Math.max(0,F.base-yB(o.backlog));const lblC=o.est?'#fcd34d':'#f8b74d';const lbl=`<text x="${x.toFixed(1)}" y="${(yB(o.backlog)-5).toFixed(1)}" fill="${lblC}" font-size="11" font-weight="700" text-anchor="middle">${_won억(o.backlog)}</text>`;const rect=o.est?`<rect x="${(x-bw/2).toFixed(1)}" y="${yB(o.backlog).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="#f59e0b" opacity="0.28" stroke="#f59e0b" stroke-width="1.3" stroke-dasharray="4 3"><title>현재(추정) ${_won억(o.backlog)} = 최근확정 ${_won억(o.base)} + 이후신규 ${_won억(o.newOrders)} − 추정납품</title></rect>`:`<rect x="${(x-bw/2).toFixed(1)}" y="${yB(o.backlog).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="#f59e0b" opacity="0.88"><title>${o.period} · ${_won억(o.backlog)}</title></rect>`;const tick=o.est?'현재':o.period;return rect+lbl+`<text x="${x.toFixed(1)}" y="${(F.base+13).toFixed(1)}" fill="${o.est?'#fcd34d':'#64748b'}" font-size="10" text-anchor="middle">${tick}</text>`;}).join('');return`<svg viewBox="0 0 ${F.W} ${F.H}" style="width:100%;height:auto;display:block;">
     <line x1="${F.PL}" y1="${F.base}" x2="${F.W-F.PR}" y2="${F.base}" stroke="#1e2d47"/>${bars}${_obTicks(F)}</svg>`;}
