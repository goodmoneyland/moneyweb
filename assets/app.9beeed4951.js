...
const evAll=data.events;const ev=evAll.slice(-30);const W=680,H=210,PL=8,PR=8,PT=14,PB=34;const iw=W-PL-PR,ih=H-PT-PB;const maxCum=Math.max(...ev.map(e=>e.cum),1);const n=ev.length;const bw=Math.max(4,Math.min(34,iw/n*0.55));const xp=bw/2+2,pw=iw-2*xp;const x=i=>PL+xp+(n===1?pw/2:pw*i/(n-1));const yC=v=>PT+ih*(1-v/maxCum);const yB=v=>PT+ih*(1-(v||0)/maxCum);const col=k=>k==='해지'?'#ef4444':(k==='정정'?'#64748b':'#38bdf8');const bars=ev.map((e,i)=>`<rect x="${(x(i)-bw/2).toFixed(1)}" y="${yB(e.amt).toFixed(1)}" width="${bw.toFixed(1)}"
       height="${Math.max(0,(PT+ih-yB(e.amt))).toFixed(1)}" rx="2" fill="${col(e.kind)}" opacity="0.85">
       <title>${e.dt} ${e.kind} ${_won억(e.amt)}${e.party?(' · '+e.party):''}</title></rect>`).join('');const linePts=ev.map((e,i)=>`${x(i).toFixed(1)},${yC(e.cum).toFixed(1)}`).join(' ');const dots=ev.map((e,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${yC(e.cum).toFixed(1)}" r="2.5" fill="#f59e0b"/>`).join('');const step=Math.max(1,Math.ceil(n/6));const xlab=ev.map((e,i)=>(i%step===0||i===n-1)?`<text x="${x(i).toFixed(1)}" y="${H-12}" fill="#64748b" font-size="11" text-anchor="middle">${e.dt.slice(2)}</text>`:'').join('');const svg=`<svg viewBox="0 0 ${W} ${H}" style="width:100%;height:auto;display:block;">
     ...
     ${dots}${xlab}
     <text x="${W-PR}" y="${PT+8}" fill="#f59e0b" font-size="11" text-anchor="end">누적 ${_won억(maxCum)}</text>
   </svg>`;box.innerHTML=`<div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:6px;">
       <div style="font-size:18px;color:#64748b;font-weight:700;letter-spacing:.04em;">수주(공급계약) 누적</div>
     ...
     <div style="background:#162033;border:1px solid #1e2d47;border-radius:8px;padding:10px 8px 2px;">${svg}
       <div style="display:flex;gap:14px;padding:6px 6px 8px;font-size:12.5px;color:#64748b;">
         <span><span style="color:#38bdf8;">▮</span> 신규</span>
         <span><span style="color:#64748b;">▮</span> 정정</span>
         <span><span style="color:#ef4444;">▮</span> 해지</span>
