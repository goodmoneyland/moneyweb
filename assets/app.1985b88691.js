...
function _obPeriodSvg(agg,F){const a=agg.slice(-24),n=a.length;if(!n)return`<svg viewBox="0 0 ${F.W} 60"><text x="20" y="34" fill="#64748b" font-size="13">집계할 신규 수주 없음</text></svg>`;const maxV=Math.max(...a.map(o=>o.amt),1);const bw=Math.max(6,Math.min(38,F.iw/n*0.64));const xp=bw/2+2,pw=F.iw-2*xp;const x=i=>F.PL+xp+(n===1?pw/2:pw*i/(n-1));const yB=v=>F.PT+F.ih*(1-v/maxV);const showLbl=n<=16;const bars=a.map((o,i)=>{const h=Math.max(0,F.base-yB(o.amt));const lbl=showLbl?`<text x="${x(i).toFixed(1)}" y="${(yB(o.amt)-4).toFixed(1)}" fill="#cbd5e1" font-size="10.5" font-weight="600" text-anchor="middle">${_won억(o.amt)}</text>`:'';return`<rect x="${(x(i)-bw/2).toFixed(1)}" y="${yB(o.amt).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="#3b82f6" opacity="0.9"><title>${o.label} · ${_won억(o.amt)}</title></rect>${lbl}`;}).join('');const step=Math.max(1,Math.ceil(n/8));const xlab=a.map((o,i)=>(i%step===0||i===n-1)?`<text x="${x(i).toFixed(1)}" y="${(F.base+12).toFixed(1)}" fill="#64748b" font-size="10" text-anchor="end" transform="rotate(-40 ${x(i).toFixed(1)} ${(F.base+12).toFixed(1)})">${o.label}</text>`:'').join('');return`<svg viewBox="0 0 ${F.W} ${F.H}" style="width:100%;height:auto;display:block;">
     <line x1="${F.PL}" y1="${F.base}" x2="${F.W-F.PR}" y2="${F.base}" stroke="#1e2d47"/>${bars}${xlab}</svg>`;}
function _obBacklogSvg(bl,F){const n=bl.length;if(!n)return`<svg viewBox="0 0 ${F.W} 60"><text x="20" y="34" fill="#64748b" font-size="13">추정 불가(계약기간 정보 부족)</text></svg>`;const maxV=Math.max(...bl.map(p=>p.v),1);const x=i=>F.PL+(n===1?F.iw/2:F.iw*i/(n-1));const y=v=>F.PT+F.ih*(1-v/maxV);const pts=bl.map((p,i)=>`${x(i).toFixed(1)},${y(p.v).toFixed(1)}`).join(' ');const area=`${F.PL},${F.base} ${pts} ${F.W-F.PR},${F.base}`;const fmtT=t=>{const d=new Date(t);return`${String(d.getFullYear()).slice(2)}.${String(d.getMonth()+1).padStart(2,'0')}`;};const step=Math.max(1,Math.ceil(n/8));const xlab=bl.map((p,i)=>(i%step===0||i===n-1)?`<text x="${x(i).toFixed(1)}" y="${(F.base+14).toFixed(1)}" fill="#64748b" font-size="10" text-anchor="middle">${fmtT(p.t)}</text>`:'').join('');return`<svg viewBox="0 0 ${F.W} ${F.H}" style="width:100%;height:auto;display:block;">
     ...
     ${xlab}</svg>`;}
function _obConfirmedSvg(ser,F){const n=ser.length;if(!n)return'';const maxV=Math.max(...ser.map(o=>o.backlog),1);const bw=Math.max(10,Math.min(52,F.iw/n*0.62));const xp=bw/2+2,pw=F.iw-2*xp;const x=i=>F.PL+xp+(n===1?pw/2:pw*i/(n-1));const yB=v=>F.PT+F.ih*(1-v/maxV);const bars=ser.map((o,i)=>{const h=Math.max(0,F.base-yB(o.backlog));const lbl=`<text x="${x(i).toFixed(1)}" y="${(yB(o.backlog)-5).toFixed(1)}" fill="#f8b74d" font-size="11" font-weight="700" text-anchor="middle">${_won억(o.backlog)}</text>`;return`<rect x="${(x(i)-bw/2).toFixed(1)}" y="${yB(o.backlog).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="#f59e0b" opacity="0.88"><title>${o.period} · ${_won억(o.backlog)}</title></rect>${lbl}`;}).join('');const xlab=ser.map((o,i)=>`<text x="${x(i).toFixed(1)}" y="${(F.base+13).toFixed(1)}" fill="#64748b" font-size="10.5" text-anchor="middle">${o.period}</text>`).join('');return`<svg viewBox="0 0 ${F.W} ${F.H}" style="width:100%;height:auto;display:block;">
     <line x1="${F.PL}" y1="${F.base}" x2="${F.W-F.PR}" y2="${F.base}" stroke="#1e2d47"/>${bars}${xlab}</svg>`;}
...
const mode=_OB_MODE[cd]||((data.confirmed&&data.confirmed.length)?'confirmed':'indiv');const k=_obKpis(data);const F={W:680,H:250,PL:10,PR:10,PT:26,PB:46};F.iw=F.W-F.PL-F.PR;F.ih=F.H-F.PT-F.PB;F.base=F.PT+F.ih;let chart='',legend='';if(mode==='month'||mode==='quarter'){chart=_obPeriodSvg(_obAgg(evAll,mode==='quarter'?'q':'m'),F);legend=`<span><span style="color:#3b82f6;">▮</span> ${mode==='quarter'?'분기':'월'}별 신규수주 합계</span>`;}else if(mode==='backlog'){chart=_obBacklogSvg(_obBacklog(evAll),F);legend=`<span><span style="color:#f59e0b;">━</span> 추정 수주잔고(계약기간 균등인식 가정)</span>`;}else if(mode==='confirmed'){chart=_obConfirmedSvg(data.confirmed||[],F);legend=`<span><span style="color:#f59e0b;">▮</span> 확정 수주잔고(정기보고서 수주현황표)</span>`;}else{const ev=evAll.slice(-24),n=ev.length;const maxAmt=Math.max(...ev.map(e=>e.amt||0),1);const maxCum=Math.max(...ev.map(e=>e.cum),1);const bw=Math.max(5,Math.min(30,F.iw/n*0.6));const xp=bw/2+2,pw=F.iw-2*xp;const x=i=>F.PL+xp+(n===1?pw/2:pw*i/(n-1));const yB=v=>F.PT+F.ih*(1-(v||0)/maxAmt);const yC=v=>F.PT+F.ih*(1-v/maxCum);const col=e=>e.kind==='해지'?'#7f1d1d':e.kind==='정정'?'#64748b':(e.ratio!=null&&e.ratio>=10)?'#ef4444':'#3b82f6';const showLbl=n<=16;const bars=ev.map((e,i)=>{const h=Math.max(0,F.base-yB(e.amt));const lbl=(showLbl&&e.amt)?`<text x="${x(i).toFixed(1)}" y="${(yB(e.amt)-4).toFixed(1)}" fill="#cbd5e1" font-size="10.5" font-weight="600" text-anchor="middle">${_won억(e.amt)}</text>`:'';return`<rect x="${(x(i)-bw/2).toFixed(1)}" y="${yB(e.amt).toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="2" fill="${col(e)}" opacity="0.9"><title>${_esc(e.dt)} ${_esc(e.kind)} ${_won억(e.amt)}${e.content?(' · '+_esc(e.content)):''}${e.party?(' / '+_esc(e.party)):''}</title></rect>${lbl}`;}).join('');const linePts=ev.map((e,i)=>`${x(i).toFixed(1)},${yC(e.cum).toFixed(1)}`).join(' ');const step=Math.max(1,Math.ceil(n/8));const xlab=ev.map((e,i)=>(i%step===0||i===n-1)?`<text x="${x(i).toFixed(1)}" y="${(F.base+12).toFixed(1)}" fill="#64748b" font-size="10" text-anchor="end" transform="rotate(-40 ${x(i).toFixed(1)} ${(F.base+12).toFixed(1)})">${e.dt.slice(2)}</text>`:'').join('');chart=`<svg viewBox="0 0 ${F.W} ${F.H}" style="width:100%;height:auto;display:block;">
       <line x1="${F.PL}" y1="${F.base}" x2="${F.W-F.PR}" y2="${F.base}" stroke="#1e2d47"/>${bars}
       <polyline points="${linePts}" fill="none" stroke="#f59e0b" stroke-width="1.5" opacity="0.85"/>${xlab}
